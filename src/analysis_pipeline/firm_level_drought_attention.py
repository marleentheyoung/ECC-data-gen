#!/usr/bin/env python3
"""
Firm-level climate drought attention calculator using semantic search.

This script calculates firm-quarter climate drought attention measures by 
applying semantic search to identify drought-related discussions in earnings calls.
...
Usage:
    ...
    python scripts/3_agg_variables/firm_level_drought_attention.py --create-annotation-sample
    
    # Validate threshold after human annotation
    python scripts/3_agg_variables/firm_level_drought_attention.py --validate-threshold annotation_sample_completed.csv
    
    # Run main analysis
    python scripts/3_agg_variables/firm_level_drought_attention.py
...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(LOGS_DIR / 'firm_level_drought_attention.log', mode='a')
        ]
    )
...
class DroughtAnalyzer(BaseSemanticSearcher):
    """
    Semantic searcher for climate Drought discussions in earnings calls.
    ...
    def __init__(self, index_path: Path, config_path: Optional[str] = None):
        """Initialize drought analyzer with semantic search capabilities."""
        super().__init__(index_path, config_path)
        
        self.drought_queries = [
        # Physical risk
        "drought prolonged dry spells heat stress water scarcity arid conditions",
        "extreme heat water shortages declining rainfall groundwater depletion",
        "river drying low reservoir levels reduced snowpack hydrological drought",

        # Business / operations impact
        "crop failure agricultural losses livestock mortality due to drought",
        "water restrictions disruption of operations supply chain shortages",
        "power generation risks hydropower reduction cooling water scarcity",

        # Adaptation / resilience
        "drought resilience adaptation strategies water conservation irrigation efficiency",
        "desalination recycling wastewater management alternative water sources",
        "drought contingency planning business continuity water stress mitigation",

        # Financial framing
        "insurance claims losses drought risk underwriting exposure",
        "drought-related write-downs stranded assets reduced valuations",
        "liability exposure litigation due to water mismanagement or scarcity",

        # Sector-specific
        "agriculture crop yield losses irrigation costs drought resilience",
        "utilities water utilities hydropower energy generation drought impacts",
        "real estate property devaluation water scarcity regional drought risk",
        "beverage food industry vulnerability to drought water supply constraints"
    ]

        
        self.similarity_threshold = 0.42
        
    def find_drought_snippets(self) -> Dict[str, List[ClimateSnippet]]:
        """Find all drought-related snippets using semantic search."""
        logger = logging.getLogger(__name__)
        
        logger.info(f"🔍 Searching for drought snippets with {len(self.drought_queries)} queries")
        logger.info(f"Using similarity threshold: {self.similarity_threshold}")
        
        # Perform semantic search
        drought_snippets = self.multi_query_search(
            queries=self.drought_queries,
            top_k=10000,  # Large number to get comprehensive results
            min_score=self.similarity_threshold,
            aggregation='max'
        )
        
        logger.info(f"Found {len(drought_snippets)} drought-related snippets")
        
        # Group snippets by earnings call
        snippets_by_call = {}
        for snippet in drought_snippets:
    ...
    def export_for_annotation(self, annotation_samples: List[ClimateSnippet], output_path: Path) -> pd.DataFrame:
        ...
        for i, snippet in enumerate(annotation_samples):
            annotation_data.append({
                ...
                'human_binary': '',     # Instructions: 0=Not drought-related, 1=drought-related
...
def create_firm_level_drought_panel(enhanced_snippets_path: Path, 
                                       structured_path: Path,
                                       stock_indices: List[str],
                                       semantic_index_path: Path) -> pd.DataFrame:
    """
    Create firm-level climate drought attention panel dataset using semantic search.
    
    Returns:
        DataFrame with firm-quarter climate drought attention measures
    """
    logger = logging.getLogger(__name__)
    
    # Initialize semantic searcher
    logger.info(f"🔍 Initializing semantic search from {semantic_index_path}")
    drought_analyzer = DroughtAnalyzer(semantic_index_path)
    
    # Find drought snippets using semantic search
    drought_snippets_by_call = drought_analyzer.find_drought_snippets()
        ...
        for json_file in tqdm(json_files, desc=f"Processing {stock_index}"):
            try:
                ...
                for transcript in data:
                    ...
                    # Check for drought content using semantic search results
                    call_key = f"{ticker}_{year}_{quarter}"
                    drought_snippets = drought_snippets_by_call.get(call_key, [])
                    
                    # Count drought sentences from semantic search
                    drought_sentence_count = len(drought_snippets)
                    
                    # Calculate similarity statistics
                    if drought_snippets:
                        similarity_scores = [s.similarity_score for s in drought_snippets]
                    ...
                    # Calculate drought attention ratio
                    climate_drought_ratio = 0.0
                    if total_sentences and total_sentences > 0:
                        climate_drought_ratio = drought_sentence_count / total_sentences
                    ...
                    record = {
                        ...
                        # Climate drought measures
                        'climate_drought_ratio': climate_drought_ratio,
                        'drought_sentence_count': drought_sentence_count,
                        'total_sentences_in_call': total_sentences,
                        'drought_snippet_count': len(drought_snippets),
                        'has_drought_content': len(drought_snippets) > 0,
                        
                        # Similarity measures
                        'max_similarity_score': max_similarity,
                        'mean_similarity_score': mean_similarity,
                        'min_similarity_score': min_similarity,
                        'similarity_threshold': drought_analyzer.similarity_threshold,
                        
                        # Coverage indicators
                        'drought_coverage_binary': 1 if len(drought_snippets) > 0 else 0,
                        'log_drought_sentences': np.log(drought_sentence_count + 1),
    ...
    logger.info(f"✅ Created drought panel with {len(df)} firm-quarter observations")
...
def add_lagged_variables(df: pd.DataFrame) -> pd.DataFrame:
    """Add lagged climate drought attention variables for econometric analysis."""
    
    df = df.sort_values(['ticker', 'year', 'quarter'])
    
    # Create lagged variables (useful for event studies)
    for lag in [1, 2, 4]:  # 1, 2, and 4 quarters
        df[f'climate_drought_ratio_lag{lag}'] = df.groupby('ticker')['climate_drought_ratio'].shift(lag)
        df[f'has_drought_content_lag{lag}'] = df.groupby('ticker')['has_drought_content'].shift(lag)
    
    # Create moving averages (useful for smoothing)
    for window in [2, 4]:
        df[f'climate_drought_ratio_ma{window}'] = (
            df.groupby('ticker')['climate_drought_ratio']
...
def create_summary_statistics(df: pd.DataFrame) -> Dict:
    ...
    summary_stats = {
        ...
        'drought_attention_distribution': {
            'mean': float(df['climate_drought_ratio'].mean()),
            'median': float(df['climate_drought_ratio'].median()),
            'std': float(df['climate_drought_ratio'].std()),
            'min': float(df['climate_drought_ratio'].min()),
            'max': float(df['climate_drought_ratio'].max()),
            'p25': float(df['climate_drought_ratio'].quantile(0.25)),
            'p75': float(df['climate_drought_ratio'].quantile(0.75)),
            'p90': float(df['climate_drought_ratio'].quantile(0.90)),
            'p99': float(df['climate_drought_ratio'].quantile(0.99))
        },
        
        'coverage_statistics': {
            'pct_calls_with_drought': float(df['has_drought_content'].mean() * 100),
            'avg_drought_sentences_per_call': float(df['drought_sentence_count'].mean()),
            'avg_total_sentences_per_call': float(df['total_sentences_in_call'].mean()),
            'avg_drought_snippets_per_call': float(df['drought_snippet_count'].mean())
        },
    ...
    for region in ['US', 'EU']:
        ...
        if len(region_data) > 0:
            summary_stats[f'{region}_statistics'] = {
                ...
                'avg_drought_attention': float(region_data['climate_drought_ratio'].mean()),
                'drought_coverage_pct': float(region_data['has_drought_content'].mean() * 100)
            }
...
def save_firm_level_results(df: pd.DataFrame, summary_stats: Dict, output_dir: Path):
    ...
    # Main panel dataset
    df.to_csv(output_dir / 'firm_level_drought_attention.csv', index=False)
    df.to_parquet(output_dir / 'firm_level_drought_attention.parquet')
    
    # Summary statistics
    with open(output_dir / 'firm_level_drought_summary_statistics.json', 'w') as f:
    ...
    for region in ['US', 'EU']:
        region_data = df[df['region'] == region]
        if len(region_data) > 0:
            region_data.to_csv(output_dir / f'firm_level_drought_attention_{region.lower()}.csv', index=False)
    ...
    try:
        ...
        df_stata.to_stata(
            output_dir / 'firm_level_drought_attention.dta',
            write_index=False,
            version=117
        )
    except Exception as e:
        # Fallback
        df.to_csv(output_dir / 'firm_level_drought_attention_for_stata.csv', index=False)
    ...
    if len(balanced_df) > 0:
        balanced_df.to_csv(output_dir / 'firm_level_drought_attention_balanced.csv', index=False)
...
def main():
    parser = argparse.ArgumentParser(
        description='Create firm-level climate drought attention panel dataset using semantic search',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create annotation sample for threshold validation
  python firm_level_drought_attention.py --create-annotation-sample
  
  # Validate threshold after human annotation
  python firm_level_drought_attention.py --validate-threshold annotation_sample_completed.csv
  
  # Run main analysis with default settings
  python firm_level_drought_attention.py
        """
    )
    ...
    parser.add_argument(
        ...
        help='Path to semantic index for drought search'
    )
    
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path("outputs/firm_level_panel/drought"),
    ...
    print("🌱 Firm-Level Climate drought Attention Panel Creator")
    print("=" * 55)
    ...
    try:
        ...
        if args.create_annotation_sample:
            ...
            drought_analyzer = DroughtAnalyzer(args.semantic_index_path)
            
            # Collect annotation samples
            annotation_samples = drought_analyzer.collect_annotation_samples(args.annotation_sample_size)
            
            # Export for annotation
            annotation_output = args.output_dir / 'annotation' / 'annotation_sample.csv'
            drought_analyzer.export_for_annotation(annotation_samples, annotation_output)
        ...
        if args.validate_threshold:
            ...
            results_df, optimal_threshold = drought_analyzer.calculate_threshold_performance(
            ...
            print(f"   • Current threshold: {drought_analyzer.similarity_threshold}")
            print(f"   • Optimal threshold: {optimal_threshold:.3f}")
            
            if abs(optimal_threshold - drought_analyzer.similarity_threshold) > 0.05:
                print(f"⚠️  Significant difference detected!")
                print(f"   Consider updating similarity_threshold in droughtAnalyzer class")
            else:
        ...
        print("🔍 Running main analysis to create firm-level drought panel...")
        
        # Create firm-level drought panel
        df = create_firm_level_drought_panel(
        ...
        if df.empty:
            print("❌ No firm-level drought data created!")
            return
        ...
        print(f"\n✅ Firm-level climate drought attention panel completed!")
        print(f"📁 Key output files:")
        print(f"   • firm_level_drought_attention.csv - Main panel dataset")
        print(f"   • firm_level_drought_attention.dta - Stata format")
        print(f"   • firm_level_drought_attention_balanced.csv - Balanced panel")
        print(f"   • firm_level_drought_summary_statistics.json - Summary stats")
        ...
        print(f"   • {df['ticker'].nunique():,} unique firms")
        print(f"   • {df['quarter_id'].nunique()} unique quarters")
        print(f"   • {df['has_drought_content'].mean():.1%} of calls contain drought content")
        print(f"   • Mean drought attention: {df['climate_drought_ratio'].mean():.4f}")
